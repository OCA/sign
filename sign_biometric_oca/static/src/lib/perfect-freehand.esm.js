...
function ce(e, t = {}) {
  ...
  for (let r = 0; r < e.length; r++) {
    ...
    U === void 0 && (U = m);
    const le = I < g ? j(I / g) : 1,
      fe = p - I < T ? c((p - I) / T) : 1;
    m = Math.max(0.01, m * Math.min(le, fe));
    const re = (r < e.length - 1 ? e[r + 1] : e[r]).vector,
  ...
  if (e.length === 1) {
    ...
    const r = R(se(e[e.length - 1].vector));
...
function me(e, t = {}) {
  var q;
  const {streamline: u = 0.5, size: x = 16, last: h = !1} = t;
  ...
  for (let c = 1; c < n.length; c++) {
    const p = h && c === j ? n[c].slice(0, 2) : K(S.point, n[c], y);
    if (ue(S.point, p)) continue;
    const g = ie(p, S.point);
    if (((D += g), c < j && !d)) {
      if (D < x) continue;
      d = !0;
    }
