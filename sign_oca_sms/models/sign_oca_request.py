...
class SignOcaRequest(models.Model):
    ...
    def _send_sign_sms(self, message, link, signer_partner, requested_by_user, partner_phone_field):
        message = self.env["ir.fields.converter"].text_from_html(message)
        template=self.env.ref("sign_oca_sms.sign_oca_sms_template_notification").with_context(
                {
                    "requested_by_user": requested_by_user,
                    "message": message,
                    "link": link,
                })
        body = template._render_field('body', signer_partner.ids, compute_lang=True)[signer_partner.id]
    ...
    def action_send_sms(self, sign_now=False, message=""):
        ...
        for signer in self.signer_ids:
            ...
            self._send_sign_sms(message, link, signer.partner_id, self.create_uid.name, signer.phone_field)        
